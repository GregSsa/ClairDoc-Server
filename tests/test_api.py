...
def test_project_conversations_can_be_created_listed_and_deleted(tmp_path: Path) -> None:
    headers = {"X-ClairDoc-Key": "test-secret"}
    with make_client(tmp_path) as client:
        ...
        listed = client.get(f"/api/v1/projects/{project_id}/conversations", headers=headers)
        fetched = client.get(
            f"/api/v1/projects/{project_id}/conversations/{conversation_id}",
            headers=headers,
        )
        memory = client.get(f"/api/v1/projects/{project_id}/memory", headers=headers)
    ...
    assert created.status_code == 201
    assert listed.json()[0]["title"] == "Factures"
    assert fetched.json()["messages"] == []
    assert memory.json()["content"].startswith("# Archives")
