...
AMOUNT_PATTERN = re.compile(r"\b\d{1,3}(?:[ .]\d{3})*(?:[,.]\d{2})?\s?(?:€|EUR)(?=\s|$)", re.I)
...
class RagService:
    ...
    def document_library(self, project_id: UUID) -> DocumentLibrary:
        ...
        for record in index.get("relationships", []):
            if not isinstance(record, dict):
                continue
            try:
                relationship = DocumentRelationship.model_validate(record)
            except ValueError:
                continue
            key = (
                str(relationship.source_job_id),
                str(relationship.target_job_id),
                relationship.kind,
                relationship.label,
            )
    ...
    def _refresh_project_memory(self, project_id: UUID, documents: list[dict[str, Any]]) -> None:
        ...
        overview = ", ".join(f"{name}: {count}" for name, count in sorted(categories.items()))
        content = (
            f"# {project.name}\n\n## Rôle du projet\n"
            f"{project.description or 'Projet documentaire ClairDoc.'}\n\n"
            f"## Dossier source\n{project.source_root or 'Non défini'}\n\n"
            f"## Vue d'ensemble\n{len(documents)} document(s) indexé(s). {overview}\n\n"
